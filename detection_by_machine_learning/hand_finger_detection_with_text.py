...
while True:
    ...
    if results.multi_hand_landmarks:
        for hand_landmarks in results.multi_hand_landmarks:
            ...
            # Display the names of open fingers
            open_finger_names = [finger_names[i] for i in range(len(fingers)) if fingers[i]]
            cv2.putText(frame, f'Open: {", ".join(open_finger_names)}', (10, 70), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2, cv2.LINE_AA)
